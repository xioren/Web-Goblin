...
parser.add_argument('--best', help='used in conjunction with --force, overwrites files only if new file size is larger', action='store_true')

parser.add_argument('-d', '--delay', help='delay ("-1" for randomized delay), default: 0', type=float, default=0)

parser.add_argument('-D', '--dir', help='specify name or path of the download directory. \
                                   the directory will be created if it does not exist', type=str, default='')

parser.add_argument('-e', '--ext', help='specify which extension(s) to download jpg,png[,...]', type=str, default='')
...
parser.add_argument('-G', '--greedy', help='find urls based on regex instead of html tags (only applies to generic goblin)', action='store_true')

parser.add_argument('-L', '--list', help='list available goblins', action='store_true')
...
parser.add_argument('-M', '--minsize', help='minimum filesize to download (in bytes), default: 30000 (30kb)', type=int, default=30000)

parser.add_argument('-m', '--mode', help='mode settings (goblin dependant)')

parser.add_argument('-n', '--nodl', help='print urls to stdout instead of downloading', action='store_true')
...
parser.add_argument('-N', '--noup', help='do not remove cropping and scaling from urls', action='store_true')

parser.add_argument('-p', '--posts', help='number of posts (n<100) to fetch (goblin dependant)', type=int, default=100)
...
parser.add_argument('-S', '--step', help='iteration step size (n)', type=int, default=1)
